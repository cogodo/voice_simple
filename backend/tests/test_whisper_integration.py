...
class TestWhisperIntegration(unittest.TestCase):
    ...
    def _create_test_audio_wav(
        self, duration_seconds=1.0, sample_rate=16000, channels=1, frequency=440
    ):
        """
        ...
        for i in range(num_samples):
            ...
            t = i / sample_rate
            sample = int(
                32767 * 0.1 * math.sin(2 * math.pi * frequency * t)
            )  # Low amplitude
...
def run_standalone_test():
    ...
    try:
        ...
        print(
            "   The test audio is just a sine wave tone, so transcription will be empty/minimal"
        )
