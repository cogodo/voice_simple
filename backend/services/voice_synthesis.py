...
if __name__ == "__main__":
    ...
    class MockFlaskLogger:
        def info(self, msg):
            logging.info(f"(MockFlaskLogger) {msg}")

        def error(self, msg, exc_info=None):
            logging.error(f"(MockFlaskLogger) {msg}", exc_info=exc_info)

        def warning(self, msg):
            logging.warning(f"(MockFlaskLogger) {msg}")

        def debug(self, msg):
            logging.debug(f"(MockFlaskLogger) {msg}")
...
def my_processing_function(text):
    ...
    if not api_key:
        ...
        return (
            "Error: CARTESIA_API_KEY environment variable not set. Please configure it."
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"--- Performing direct DNS lookup for {cartesia_hostname}:{cartesia_wss_port} using socket.getaddrinfo (Python) ---"
        )
        addr_info_results = socket.getaddrinfo(
            cartesia_hostname, cartesia_wss_port, 0, socket.SOCK_STREAM
        )
        current_app.logger.info(
            f"socket.getaddrinfo for {cartesia_hostname} SUCCEEDED. Results: {addr_info_results}"
        )
    except socket.gaierror as e_direct_gaierror:
        current_app.logger.error(
            f"Direct socket.getaddrinfo for {cartesia_hostname} FAILED: {e_direct_gaierror}",
            exc_info=True,
        )
        ...
        current_app.logger.error(
            f"Direct socket.getaddrinfo for {cartesia_hostname} FAILED with unexpected error: {e_direct_other}",
            exc_info=True,
        )
    ...
    try:
        ...
        target_ws_url = (
            ws.ws_url
        )  # This should be something like wss://api.cartesia.ai/tts/v1/websocket
        current_app.logger.info(
            f"Cartesia client is targeting WebSocket URL: {target_ws_url}"
        )
        ...
        if hostname_to_resolve != cartesia_hostname:
            current_app.logger.warning(
                f"Mismatch! Direct test used '{cartesia_hostname}' but client targets '{hostname_to_resolve}'. This might be an issue."
            )

        current_app.logger.info(
            f"Attempting to connect to Cartesia WebSocket (hostname: '{hostname_to_resolve}') via client library..."
        )
        ws.connect()
        current_app.logger.info(
            "Cartesia WebSocket connected successfully via client library."
        )
        ...
        for i, output_item in enumerate(
            ...
            try:
                current_app.logger.debug(
                    f"Stream item {i} attributes: {dir(output_item)}"
                )
            ...
            if hasattr(output_item, "audio") and output_item.audio is not None:
                current_app.logger.info(
                    f"Stream item {i}: Received audio chunk of length {len(output_item.audio)}"
                )
                audio_chunks.append(output_item.audio)
            else:
                current_app.logger.info(
                    f"Stream item {i}: No audio data in this item or audio attribute is None."
                )

            if hasattr(output_item, "status") and output_item.status is not None:
                current_app.logger.info(
                    f"Stream item {i}: Status present - Code: {getattr(output_item.status, 'code', 'N/A')}, Message: {getattr(output_item.status, 'message', 'N/A')}"
                )
            elif (
                hasattr(output_item, "event_type")
                and output_item.event_type is not None
            ):
                current_app.logger.info(
                    f"Stream item {i}: Event type present - {output_item.event_type}"
                )
        ...
        current_app.logger.error(
            f"Network DNS Resolution Error (socket.gaierror) during client.ws.connect(): {e}. Target URL '{target_ws_url}' (hostname: '{hostname_to_resolve}')",
            exc_info=True,
        )
        return f"Error: Network DNS resolution failed for host '{hostname_to_resolve}' when using client library. Details: {e}"
    except RuntimeError as e:
        current_app.logger.error(
            f"RuntimeError during client.ws.connect() or operation (hostname: '{hostname_to_resolve}'): {e}",
            exc_info=True,
        )
        ...
        current_app.logger.error(
            f"An unexpected error occurred (hostname: '{hostname_to_resolve}'): {e}",
            exc_info=True,
        )
    ...
    if not audio_chunks:
        current_app.logger.warning(
            "No audio chunks received from Cartesia. The audio_chunks list is empty."
        )
        return "Error: No audio data was received from the voice synthesis service."

    current_app.logger.info(
        f"Total audio chunks received: {len(audio_chunks)}. Concatenating..."
    )
    full_audio_bytes_f32le = b"".join(audio_chunks)
    current_app.logger.info(
        f"Total concatenated float32 audio bytes: {len(full_audio_bytes_f32le)}."
    )

    if len(full_audio_bytes_f32le) == 0:
        current_app.logger.warning(
            "Concatenated audio data is empty. Cannot create WAV."
        )
    ...
    if all_float_samples:
        ...
        rms_level = (
            sum(x * x for x in all_float_samples) / len(all_float_samples)
        ) ** 0.5
        ...
        if max_level > 0:
            ...
            optimal_gain = max(
                1.0, min(optimal_gain, 8.0)
            )  # Limit gain between 1x and 8x

            current_app.logger.info("🎚️ Audio Analysis Results:")
            current_app.logger.info(
                f"   • Original Peak: {max_level:.4f} ({20 * math.log10(max_level) if max_level > 0 else -100:.1f}dB)"
            )
            current_app.logger.info(
                f"   • Original RMS: {rms_level:.4f} ({20 * math.log10(rms_level) if rms_level > 0 else -100:.1f}dB)"
            )
            current_app.logger.info(
                f"   • Optimal Gain: {optimal_gain:.2f}x ({20 * math.log10(optimal_gain):.1f}dB boost)"
            )
        else:
            optimal_gain = 4.0  # Default gain if no signal detected
            current_app.logger.warning(
                "⚠️ No audio signal detected, using default 4x gain"
            )
    else:
        optimal_gain = 4.0
    ...
    current_app.logger.info(
        f"Total concatenated int16 audio bytes: {len(full_audio_bytes_s16le)} (with {optimal_gain:.2f}x gain)."
    )

    output_filename = "generated_speech.wav"
    current_app.logger.info(
        f"Attempting to save audio to server file: {output_filename}"
    )
    ...
    try:
        ...
        current_app.logger.info(
            f"Successfully saved audio to {output_filename} on the server."
        )

        current_app.logger.info(
            f"Creating WAV in memory for base64 encoding. Total int16 bytes: {len(full_audio_bytes_s16le)}, Sample rate: {sample_rate}"
        )
        ...
        current_app.logger.info(
            f"Successfully created audio data URI (length: {len(data_uri)})."
        )
        return data_uri

    except Exception as e:
        current_app.logger.error(
            f"Error saving WAV file to disk or during base64 encoding: {e}",
            exc_info=True,
        )
...
def my_processing_function_streaming(text: str, logger) -> Generator[bytes, None, None]:
    ...
    try:
        ...
        logger.info(
            f"🎵 Starting real-time streaming with IIR smoothing (α={filter_alpha}, gain={gentle_gain}x)..."
        )
                ...
                if hasattr(item, "data") and isinstance(item.data, str):
                    try:
                        ...
                        if len(audio_bytes_f32le) > 0:
                            ...
                            for i in range(num_samples):
                                float_val = struct.unpack_from(
                                    "<f", audio_bytes_f32le, i * 4
                                )[0]
                                ...
                                # y[n] = α * x[n] + (1-α) * y[n-1]
                                filter_state = (
                                    filter_alpha * gained_val
                                    + (1 - filter_alpha) * filter_state
                                )

                                # Soft clipping with gentle saturation
                                if filter_state > 1.0:
                                    smoothed_val = 1.0 - math.exp(
                                        -(filter_state - 1.0)
                                    )  # Soft clip positive
                                elif filter_state < -1.0:
                                    smoothed_val = -1.0 + math.exp(
                                        -(abs(filter_state) - 1.0)
                                    )  # Soft clip negative
                                else:
                                ...
                                int_val = max(
                                    -32768, min(32767, int_val)
                                )  # Hard limit for safety

                                struct.pack_into(
                                    "<h", audio_bytes_s16le, i * 2, int_val
                                )
                        ...
                        logger.error(
                            f"Error decoding base64 audio data: {decode_error}"
                        )
                        continue
        ...
        logger.info(
            f"✅ Real-time streaming with IIR smoothing completed: {chunk_count} chunks, {total_bytes} bytes"
        )
...
def diagnose_cartesia_audio_quality(text: str, logger) -> dict:
    ...
    try:
        ...
        dynamic_range_db = (
            20 * math.log10(max_level / min_level) if min_level > 0 else float("inf")
        )
        ...
        medium_samples = len(
            [x for x in abs_samples if quiet_threshold <= x < medium_threshold]
        )
        loud_samples = len(
            [x for x in abs_samples if medium_threshold <= x < loud_threshold]
        )
        ...
        # Add issue detection
        if max_level < 0.1:
            diagnosis["recommendations"]["issues"].append(
                "Very low audio levels detected"
            )
        if clipping_percentage > 1.0:
            diagnosis["recommendations"]["issues"].append(
                f"Clipping detected in {clipping_percentage:.1f}% of samples"
            )
        if dynamic_range_db < 6:
            diagnosis["recommendations"]["issues"].append("Poor dynamic range (< 6dB)")
        if rms_level < 0.05:
            diagnosis["recommendations"]["issues"].append(
                "Very low RMS level - audio may be too quiet"
            )

        logger.info("✅ Cartesia audio diagnosis completed")
        return diagnosis
...
if __name__ == "__main__":
    ...
    if "current_app" not in globals():
        ...
        class MockFlaskLogger:
            def info(self, msg):
                logging.info(f"(MockVoiceThingLogger) {msg}")

            def error(self, msg, exc_info=None):
                logging.error(f"(MockVoiceThingLogger) {msg}", exc_info=exc_info)

            def warning(self, msg):
                logging.warning(f"(MockVoiceThingLogger) {msg}")

            def debug(self, msg):
                logging.debug(f"(MockVoiceThingLogger) {msg}")
        ...
        current_app = (
            MockCurrentApp()
        )  # Assign to global current_app for my_processing_function

    logging.info(
        "Starting direct test of my_processing_function (the non-streaming one)..."
    )
    ...
    if not api_key_exists:
        logging.critical(
            "CRITICAL: CARTESIA_API_KEY environment variable is not set. Non-streaming test cannot run."
        )
    else:
        logging.info(
            "CARTESIA_API_KEY is set. Proceeding with test call using text 'Hello from direct test' (non-streaming)"
        )
        ...
        if isinstance(result, str) and result.startswith("data:audio/wav;base64,"):
            logging.info(
                f"Generated data URI successfully (first 100 chars): {result[:100]}..."
            )
            if os.path.exists("generated_speech.wav"):
                logging.info(
                    "File 'generated_speech.wav' was also created successfully on the server."
                )
            else:
                logging.warning(
                    "File 'generated_speech.wav' was NOT created on the server during direct test."
                )
        else:
            logging.error(
                f"Error or unexpected result in non-streaming direct test: {result}"
            )
