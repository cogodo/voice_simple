...
class AudioDiagnostics:
    ...
    def test_cartesia_quality(self):
        ...
        try:
            ...
            self.cartesia_analysis = diagnose_cartesia_audio_quality(
                self.test_text, logger
            )
            ...
            print("✅ Cartesia Raw Audio Analysis:")
            print(
                f"   • Peak Level: {audio_levels['peak_level']:.4f} ({audio_levels['peak_level_db']:.1f}dB)"
            )
            print(
                f"   • RMS Level: {audio_levels['rms_level']:.4f} ({audio_levels['rms_level_db']:.1f}dB)"
            )
            ...
            if audio_levels["dynamic_range_db"] < 6:
                self.artifacts_detected.append(
                    "Raw Cartesia output has poor dynamic range"
                )
    ...
    def test_backend_processing(self):
        ...
        try:
            ...
            if not iir_chunks:
                self.artifacts_detected.append(
                    "No chunks received from backend IIR processing"
                )
                return
            ...
            self.backend_analysis = self.analyze_pcm_audio(
                combined_audio, "Backend IIR Output"
            )
            ...
            print(
                f"✅ Backend IIR processing complete: {len(combined_audio)} bytes, {chunk_count} chunks"
            )
    ...
    def test_float_to_int_conversion(self):
        ...
        for name, level in test_signals:
            ...
            max_error = max(
                abs(orig - conv) for orig, conv in zip(float_samples, converted_back)
            )
            rms_error = math.sqrt(
                sum(
                    (orig - conv) ** 2
                    for orig, conv in zip(float_samples, converted_back)
                )
                / len(float_samples)
            )

            print(
                f"      {name}: Max error: {max_error:.6f}, RMS error: {rms_error:.6f}"
            )

            if max_error > 0.0001:  # Check for excessive quantization error
                self.artifacts_detected.append(
                    f"Float32→Int16 conversion error too high for {name}"
                )
    ...
    def test_frame_boundary_issues(self):
        ...
        if hasattr(self, "processed_backend_data") and self.processed_backend_data:
            ...
            for i in range(0, len(self.processed_backend_data), 2):
                if i + 1 < len(self.processed_backend_data):
                    sample = struct.unpack(
                        "<h", self.processed_backend_data[i : i + 2]
                    )[0]
            ...
            if jump_rate > 0.1:  # More than 0.1% sudden jumps
                self.artifacts_detected.append(
                    f"Frame boundary discontinuities detected: {jump_rate:.2f}% jump rate"
                )
    ...
    def test_sample_rate_issues(self):
        ...
        if self.sample_rate != expected_rate:
            self.artifacts_detected.append(
                f"Sample rate mismatch: expected {expected_rate}, got {self.sample_rate}"
            )
    ...
    def test_endianness_issues(self):
        ...
        for value in test_values:
            ...
            if unpacked_le != (value if value < 32768 else value - 65536):
                self.artifacts_detected.append(
                    f"Endianness issue detected with value 0x{value:04X}"
                )
    ...
    def check_iir_artifacts(self, audio_data):
        ...
        for i in range(2, len(samples) - 2):
            ...
            if (
                ...
                and samples[i] * samples[i + 1] < 0
                and abs(samples[i]) > 0.1
            ):
                oscillation_count += 1

        if oscillation_count > len(samples) * 0.01:  # More than 1% oscillations
            self.artifacts_detected.append(
                f"IIR filter instability detected: {oscillation_count} oscillations"
            )
        ...
        if (
            high_freq_content < 0.01
        ):  # Very low HF content might indicate over-smoothing
            self.artifacts_detected.append(
                "Possible over-smoothing: very low high-frequency content"
            )
    ...
    def analyze_pcm_audio(self, audio_data, name):
        ...
        print(
            f"   • Duration: {analysis['duration_seconds']:.2f}s ({analysis['total_samples']} samples)"
        )
    ...
    def analyze_artifact_sources(self):
        ...
        print("📊 Potential artifact sources identified:")
        ...
        if not detected_sources:
            ...
            print(
                "   🔍 Artifacts likely occur in frontend Web Audio API or audio scheduling"
            )
    ...
    def generate_recommendations(self):
        ...
        if "over-smoothing" in str(self.artifacts_detected).lower():
            recommendations.append(
                "Reduce IIR filter alpha (increase from 0.35 to 0.5+) for more audio clarity"
            )

        if "instability" in str(self.artifacts_detected).lower():
            recommendations.append(
                "Check IIR filter stability - consider reducing gain or alpha value"
            )

        if "conversion error" in str(self.artifacts_detected).lower():
            recommendations.append(
                "Improve Float32→Int16 conversion precision or use higher bit depth"
            )

        if "boundary discontinuities" in str(self.artifacts_detected).lower():
            recommendations.append(
                "Add crossfading between audio frames to eliminate boundary artifacts"
            )

        if "web_audio" in str(self.artifacts_detected).lower():
            recommendations.append(
                "Investigate Web Audio API sample rate matching and scheduling precision"
            )

        if not self.artifacts_detected:
            recommendations.append(
                "Backend audio processing appears clean - focus on frontend Web Audio implementation"
            )
            recommendations.append(
                "Check Web Audio API createBuffer sample rate consistency"
            )
            recommendations.append("Verify audio scheduling timing precision")
            recommendations.append(
                "Test with different frame sizes or buffering strategies"
            )
    ...
    def print_diagnostic_summary(self, report):
        ...
        print("\n📊 Key Metrics:")
        if (
            "cartesia_analysis" in report
            and "audio_levels" in report["cartesia_analysis"]
        ):
            levels = report["cartesia_analysis"]["audio_levels"]
            print(
                f"   • Cartesia Peak: {levels['peak_level']:.3f} ({levels['peak_level_db']:.1f}dB)"
            )
            print(
                f"   • Cartesia RMS: {levels['rms_level']:.3f} ({levels['rms_level_db']:.1f}dB)"
            )

        if "backend_analysis" in report and report["backend_analysis"]:
            backend = report["backend_analysis"]
            print(
                f"   • Backend Peak: {backend['peak_level']:.3f} ({backend['peak_db']:.1f}dB)"
            )
            print(
                f"   • Backend RMS: {backend['rms_level']:.3f} ({backend['rms_db']:.1f}dB)"
            )
...
def main():
    ...
    print(
        "This tool will test every stage of the audio pipeline to identify artifact sources."
    )
    print("")
