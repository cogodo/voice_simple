...
class AudioStreamer:
    ...
    def save_and_play_complete_audio(self, filename="test_streaming_output.wav"):
        ...
        try:
            ...
            print(
                f"📊 Audio info: {len(self.audio_buffer)} bytes, {len(self.audio_buffer) / 2 / self.sample_rate:.2f} seconds"
            )
    ...
    def play_audio_file(self, filename):
        ...
        try:
            if sys.platform == "darwin":  # macOS
                ...
                print("🔊 Playing audio with built-in player...")
    ...
    def create_real_time_chunks(self, chunk_size_seconds=1.0):
        ...
        chunk_size_bytes = int(
            self.sample_rate * 2 * chunk_size_seconds
        ...
        while len(self.audio_buffer) >= chunk_size_bytes:
            ...
            try:
                ...
                threading.Thread(
                    target=self._play_chunk_async, args=(temp_file,), daemon=True
                ).start()

                print(
                    f"🎵 Playing real-time chunk {chunk_num} ({len(chunk_data)} bytes)"
                )
...
def test_streaming():
    ...
    print("🎯 Testing TTS streaming with AUDIO PLAYBACK")
    print(f'📝 Text: "{test_text}"')
    print("⏱️  Starting streaming test...\n")
    ...
    try:
        ...
        print(
            f"   • Avg chunk size: {total_bytes / chunk_count if chunk_count > 0 else 0:.0f} bytes"
        )

        if chunk_count == 0:
            print(
                "⚠️ Warning: No audio chunks received. This might indicate an API issue."
            )
            return False
        ...
        if success:
            ...
            print("💡 Listen carefully - the audio should be clear without static!")
