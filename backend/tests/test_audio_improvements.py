...
def test_audio_improvements():
    ...
    try:
        ...
        print(f"   • Peak Level: {diagnosis['audio_levels']['peak_level']:.4f} ({diagnosis['audio_levels']['peak_level_db']:.1f}dB)")
        print(f"   • RMS Level: {diagnosis['audio_levels']['rms_level']:.4f} ({diagnosis['audio_levels']['rms_level_db']:.1f}dB)")
        print(f"   • Recommended Gain: {diagnosis['recommendations']['recommended_gain']:.2f}x ({diagnosis['recommendations']['recommended_gain_db']:.1f}dB)")
        ...
        if diagnosis['recommendations']['recommended_gain'] > 1.0:
            print(f"🔧 Audio improvements detected:")
            print(f"   • Gain boost applied: +{diagnosis['recommendations']['recommended_gain_db']:.1f}dB")
            print(f"   • This should resolve low audio level issues")
