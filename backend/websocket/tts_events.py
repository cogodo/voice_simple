...
def register_tts_events(socketio, app):
    ...
    def handle_start_tts(data):
        ...
        try:
            ...
            app.logger.info(
                f"Starting TTS streaming for session {session_id}, text: '{text[:50]}...'"
            )
            ...
            socketio.start_background_task(
                stream_tts_pcm_timed, text, session_id, socketio, app, active_streams
            )
    ...
    def stream_tts_pcm_timed(
        text, session_id, socketio_instance, app_instance, stream_tracker
    ):
        """Stream TTS audio as raw PCM frames in real-time."""
        try:
            app_instance.logger.info(
                f"Starting real-time PCM TTS streaming for session {session_id}"
            )

            # Signal start of streaming to specific client
            socketio_instance.emit(
                "tts_started", {"status": "streaming"}, room=session_id
            )
            ...
            try:
                for frame_data in my_processing_function_streaming(
                    text, app_instance.logger
                ):
                    # Check if stream should stop
                    if stream_state["should_stop"]:
                        app_instance.logger.info(
                            f"Stream stopped at frame {frame_count} for session {session_id}"
                        )
                        break

                    # Send frame immediately as it's generated
                    socketio_instance.emit(
                        "pcm_frame", list(frame_data), room=session_id
                    )
                    frame_count += 1
                ...
                socketio_instance.emit(
                    "tts_error",
                    {"error": f"Real-time streaming failed: {str(e)}"},
                    room=session_id,
                )
                return
            ...
            app_instance.logger.error(
                f"Error in real-time TTS streaming for session {session_id}: {e}",
                exc_info=True,
            )
    ...
    def handle_audio_buffer_status(data):
        ...
        try:
            ...
            app.logger.debug(
                f"Client {session_id} buffer status: {buffer_size}ms, underruns: {underrun_count}"
            )
