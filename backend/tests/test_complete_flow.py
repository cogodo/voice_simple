...
def test_complete_flow():
    ...
    try:
                ...
                while True:
                    ...
                    if events:
                        ...
                        if event_name == 'audio_chunk':
                            ...
                            print(f"  📢 Audio chunk received: {len(data.get('audio_chunk', []))} bytes")
                        
                        elif event_name == 'tts_starting':
                            print(f"  🎵 TTS starting: {data.get('text', '')[:30]}...")
                        
                        elif event_name == 'tts_finished':
                            print(f"  ✅ TTS finished: {data.get('total_chunks', 0)} chunks")
                            tts_finished = True
                            break
        ...
        # Send TTS request
        sio.emit('synthesize_speech_streaming', {'text': 'Hello, this is a test of the text to speech system.'})
        
        # Track audio chunks
        track_audio_chunks()
        
        if audio_chunks_received and tts_finished:
            print(f"✅ Direct TTS working: {len(audio_chunks_received)} audio chunks received")
        else:
            print(f"❌ Direct TTS issue: {len(audio_chunks_received)} chunks, finished: {tts_finished}")
                ...
                while True:
                    ...
                    if events:
                        ...
                        if event_name == 'ai_thinking':
                            ...
                            print(f"  🎵 Auto-TTS starting for AI response...")
                        
                        elif event_name == 'tts_finished':
                            print(f"  ✅ Auto-TTS finished: {data.get('total_chunks', 0)} chunks")
                            tts_finished = True
                            break
        ...
        sio.emit('conversation_text_input', {'text': 'Please tell me a fun fact about space in one sentence.'})
        
        # Track conversation flow
        track_conversation()
        
        if ai_response_received and audio_chunks_received and tts_finished:
            print(f"✅ AI Conversation + Auto-TTS working: Response received, {len(audio_chunks_received)} audio chunks")
        else:
            print(f"❌ AI Conversation issue: AI response: {ai_response_received}, Audio chunks: {len(audio_chunks_received)}, TTS finished: {tts_finished}")
        ...
        print(f"{'✅' if audio_chunks_received else '❌'} Audio Streaming: {'Working' if audio_chunks_received else 'Failed'}")
        print(f"{'✅' if ai_response_received else '❌'} AI Responses: {'Working' if ai_response_received else 'Failed'}")
        print(f"{'✅' if tts_finished else '❌'} Auto-TTS after AI: {'Working' if tts_finished else 'Failed'}")
